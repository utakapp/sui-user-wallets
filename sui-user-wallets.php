...
class Sui_User_Wallets {
    ...
    private function __construct() {
        ...
        add_action('wp_ajax_suw_dismiss_v106_notice', array($this, 'ajax_dismiss_v106_notice'));
    ...
    public function check_database_table() {
        ...
        $dismissed = get_option('suw_v106_notice_dismissed', false);
        if (!$dismissed && version_compare(SUW_VERSION, '1.0.6', '>=')) {
            ?>
            <div class="notice notice-success is-dismissible" data-dismissible="suw-v106-notice">
                <p>
                    <strong>🎉 Sui User Wallets v1.0.6:</strong>
                    Auto-Update erfolgreich! Neue Dokumentation: DEVELOPER_NOTES.md hinzugefügt.
                    <a href="https://github.com/utakapp/sui-user-wallets/releases/tag/v1.0.6" target="_blank">Release Notes</a>
                </p>
            </div>
            <script>
            jQuery(document).ready(function($) {
                $(document).on('click', '[data-dismissible="suw-v106-notice"] .notice-dismiss', function() {
                    $.post(ajaxurl, {
                        action: 'suw_dismiss_v106_notice'
                    });
                });
            });
            </script>
    ...
    // AJAX: v1.0.6 Notice dismissal
    public function ajax_dismiss_v106_notice() {
        if (!current_user_can('manage_options')) {
            wp_die();
        }
        update_option('suw_v106_notice_dismissed', true);
        wp_die();
    }
